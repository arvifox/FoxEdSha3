package com.arvifox.libed.ed25519.math;

import java.io.Serializable;
import java.util.Arrays;
import com.arvifox.libed.ed25519.Utils;
...
public class GroupElement implements Serializable {
        ...
        switch (repr) {
          ...
          case P3:
            return p3(this.curve, this.X.multiply(this.T), Y.multiply(this.Z),
                this.Z.multiply(this.T), this.X.multiply(this.Y), false);
          case P3PrecomputedDouble:
            return p3(this.curve, this.X.multiply(this.T), Y.multiply(this.Z),
                this.Z.multiply(this.T), this.X.multiply(this.Y), true);
          case P1P1:
  ...
  private GroupElement[][] precomputeSingle() {
    ...
    for (int i = 0; i < 32; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        precmp[i][j] = precomp(this.curve, y.add(x), y.subtract(x),
  ...
  private GroupElement[] precomputeDouble() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      dblPrecmp[i] = precomp(this.curve, y.add(x), y.subtract(x),
          x.multiply(y).multiply(this.curve.get2D()));
   ...
   * @return the GroupElement
   */
  public GroupElement doubleScalarMultiplyVariableTime(final GroupElement A, final byte[] a,
      final byte[] b) {
